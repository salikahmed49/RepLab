...
export function Badge({ className, variant = "default", ...props }: React.HTMLAttributes<HTMLSpanElement> & { variant?: "default" | "secondary" | "outline" }) {
  return <span className={cn("inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold", variant === "secondary" && "border-transparent bg-secondary text-secondary-foreground", variant === "outline" && "bg-background text-foreground", variant === "default" && "border-transparent bg-primary text-primary-foreground", className)} {...props} />;
}
